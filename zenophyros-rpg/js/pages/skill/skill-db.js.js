window.SkillsDB = (() => {
  ...
  function filter(filtersData) {
    ...
    return skills.filter((skill) => {
      ...
      const matchQuery =
        !query || matchName || matchSummary || matchTags;

      const matchClass =
        !skillClass || skill.class === skillClass;

      const matchSpecies =
        !species ||
        skill.requirements?.species?.includes(species);

      const matchActivation =
        !activation || skill.activation === activation;

      const matchOrigin =
        !origin || skill.origin === origin;

      const matchTypes =
        !types || skill.types?.includes(types);
